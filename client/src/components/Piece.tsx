...
import { effectivePieceLabel, effectivePieceType, isSwapAffected } from '../../../shared/xiangqiRules';
...
export default function Piece({ piece, style, theme, game }: { piece: PieceModel; style: 'asset' | 'han' | 'vi'; theme: any; game?: any }) {
  ...
  if (style === 'asset') {
    ...
    return <span className={cls} title={`${pieceNameVi[piece.type]}${swapTitle}`} style={{ color: piece.color === 'red' ? theme.redPieceColor : theme.blackPieceColor }}>
      ...
      <span className={`piece-symbol-overlay ${piece.color}`}>{han[piece.color][piece.type]}</span>
      <span className="piece-overlay-letter">{comboLabel}</span>
    </span>;
  }

  const text = style === 'han' ? han[piece.color][piece.type] : viShort[piece.type];
  return <span className={cls} title={`${pieceNameVi[piece.type]}${swapTitle}`} style={{ color: piece.color === 'red' ? theme.redPieceColor : theme.blackPieceColor }}>{text}{swapped && <small>{comboLabel}</small>}</span>;
}
