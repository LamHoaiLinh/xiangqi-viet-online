...
export default function Piece({ piece, style, theme, game }: { piece: PieceModel; style: 'asset' | 'han' | 'vi'; theme: any; game?: any }) {
  if (piece.hidden) return <span className="piece-fallback piece-hidden" style={{ color: piece.color === 'red' ? theme.redPieceColor : theme.blackPieceColor }}>Úp</span>;
  ...
  if (style === 'asset') {
    ...
    return <span className={cls} title={`${pieceNameVi[piece.type]}${swapTitle}`} style={{ color: piece.color === 'red' ? theme.redPieceColor : theme.blackPieceColor }}><img className="piece-img" src={src} alt={pieceNameVi[piece.type]} draggable={false} onError={e => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }}/><span className="piece-overlay-letter">{swapped ? `${pieceLetterVi[piece.type]}→${pieceLetterVi[effective]}` : ''}</span></span>;
  }
