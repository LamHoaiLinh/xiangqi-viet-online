...
export default function RoomCreateModal({ socket, playerId, onClose }: { socket: Socket | null; playerId: string; onClose: () => void }) {
  ...
  const submit = () => { localStorage.setItem('xiangqi_viet_name', displayName); socket?.emit('room:create', { name, displayName, playerId, side, isPublic, allowSpectators, password: isPublic ? '' : password, timeControl, theme, gameMode, darkOptions }); onClose(); };
  return <div className="modal-backdrop"><div className="modal wide"><h2>Tạo bàn mới</h2><div className="grid-2"><label>Tên bàn<input value={name} onChange={e => setName(e.target.value)} maxLength={40}/></label><label>Tên của bạn<input value={displayName} onChange={e => setDisplayName(e.target.value)} maxLength={20}/></label><label>Chế độ chơi<select value={gameMode} onChange={e => setGameMode(e.target.value as GameMode)}><option value="xiangqi">Cờ Tướng</option><option value="dark">Cờ Úp</option></select></label><label>Chọn bên<select value={side} onChange={e => setSide(e.target.value)}><option value="red">Cầm Đỏ</option><option value="black">Cầm Đen</option><option value="auto">Tự động</option></select></label><label>Loại bàn<select value={isPublic ? 'public' : 'private'} onChange={e => setPublic(e.target.value === 'public')}><option value="public">Công khai</option><option value="private">Riêng tư</option></select></label></div>{gameMode === 'dark' && <section className="card inner"><h3>Tùy chọn Cờ Úp</h3><p className="hint">Quân còn úp đi theo vai trò ô ban đầu. Khi di chuyển hoặc ăn quân thì lật lên. Sau khi lật, các quân ngoài Tướng được qua sông/ra khỏi cung theo luật Cờ Úp thực dụng.</p><div className="grid-2"><label>Đỏ chọn hoán đổi<select value={darkOptions.redSwap} onChange={e => setDarkOptions(v => ({ ...v, redSwap: e.target.value as DarkSwapMode }))}>{swapOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></label><label>Đen chọn hoán đổi<select value={darkOptions.blackSwap} onChange={e => setDarkOptions(v => ({ ...v, blackSwap: e.target.value as DarkSwapMode }))}>{swapOptions.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></label></div><p className="hint">Ví dụ chọn Mã ⇄ Sĩ: quân Mã đã lật sẽ đi như Sĩ, quân Sĩ đã lật sẽ đi như Mã. Quân còn úp chưa áp dụng hoán đổi.</p></section>}{!isPublic && <label>Mật khẩu bàn<input value={password} onChange={e => setPassword(e.target.value)} placeholder="Để trống nếu chỉ muốn riêng tư bằng link"/></label>}<label className="check"><input type="checkbox" checked={allowSpectators} onChange={e => setAllowSpectators(e.target.checked)}/> Cho người khác quan sát</label><TimeControlPicker value={timeControl} onChange={setTimeControl}/><ThemeCustomizer theme={theme} onChange={setTheme}/><div className="actions"><button className="secondary" onClick={onClose}>Hủy</button><button onClick={submit}>Tạo bàn</button></div></div></div>;
}
