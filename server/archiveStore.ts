import { GameMode } from '../shared/gameTypes.js';
import { GameArchiveRecord, Room } from './types.js';
...
export function exportArchiveBackup() {
  ...
  return {
    ...
    exportedAt: Date.now(),
    note: 'File JSON này dùng để nhập lại toàn bộ ván Cờ Tướng/Cờ Úp đã lưu nếu Render restart hoặc redeploy làm mất RAM.',
    counts: {
...
function normalizeRecord(raw: any): GameArchiveRecord | null {
  ...
  return {
    ...
    mode,
    darkOptions: {
      redSwap: raw?.darkOptions?.redSwap === 'horse_advisor' || raw?.darkOptions?.redSwap === 'cannon_elephant' ? raw.darkOptions.redSwap : 'none',
      blackSwap: raw?.darkOptions?.blackSwap === 'horse_advisor' || raw?.darkOptions?.blackSwap === 'cannon_elephant' ? raw.darkOptions.blackSwap : 'none'
    },
